...
function extractPrompt(text: string): string {
    ...
    const cleaned = text
        .replace(/^(generate|create|make|draw|show me|paint|give me)\s+(an?\s+)?(image|picture|photo)\s+(of\s+)?/i, '')
        .trim();
...
async function generateImages(
    ...
    try {
        ...
        const statusText = isRegeneration 
            ...
            : "🎨 **Generating 1 variants...** Please wait.";
        ...
        // Prepare prompt
        const finalPrompt = `${promptText} (Generate in landscape 16:9 aspect ratio unless stated otherwise before this) (Follow the prompt exactly as stated, do not make any of your own modifications.)`;
